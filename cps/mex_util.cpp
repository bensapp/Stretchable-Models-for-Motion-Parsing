...
#include "mex_sigint_handler.cpp" //for ctrl+C handling; does not seem to be slower
...
bool isField(const mxArray*A,string field){
    return mxGetFieldNumber(A,field.c_str())>=0;
    /*
...
bool isField2(const mxArray*A,const char *fname){
    return mxGetFieldNumber(A,fname)>=0;
    /*
    int n=mxGetNumberOfFields(A);
    for(int i=0;i<n;i++){
...
int* createArray(int n,int val){
    //TODO:remove all those kind of functions...
	if(n==0)
		return NULL;
...
mxArray*createStructArray(int n){
	const char **field_names = (const char **)mxCalloc(0, sizeof(char*));
	mxArray*A=mxCreateStructMatrix(n,1,0,field_names);
	mxFree(field_names);
	return A;
}
mxArray*createStructArray(int n, vector<string>&fields){//MODIF:added &
...
void compute_cell_strings(const mxArray *A,vector<char*>&strings,bool isValid_single){
    //caution:need to do mxFree(strings) after usage
    if(mxIsCell(A)){
        int n=mxGetNumberOfElements(A);
        strings.resize(n);        
        for(int s=0;s<n;s++){
