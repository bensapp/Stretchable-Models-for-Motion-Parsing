...
if fastMode
    rvec = intconvert(options.computeX);
    ...
    if isfield(options,'v0')
        options.v0(1)=options.v0(1);%assures options.v0 will not be modified in place
        info = intconvert(1); % use resid as starting vector
        resid = options.v0;
    else
        info = intconvert(0); % use a random starting vector
        resid = zeros(n,1);
    end
...
ldv = intconvert(size(v,1));
ipntr = intconvert(zeros(15,1));
workd = zeros(n,3);
lworkl = p*(p+8);
workl = zeros(lworkl,1);
lworkl = intconvert(lworkl);
d = zeros(k,1);

ido = intconvert(0); % reverse communication parameter
nev = intconvert(k); % number of eigenvalues requested
ncv = intconvert(p); % number of Lanczos vectors
iparam = intconvert(zeros(11,1));
iparam([1 3 7]) = intconvert([1 maxit 1]);
select = intconvert(zeros(p,1));
int32_n = intconvert(n);
